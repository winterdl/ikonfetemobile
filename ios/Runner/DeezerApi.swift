...
//  Runner
//
//  Created by Tomi Alagbe on 25/03/2019.
...
//    var trackRequest: DZRCancelable?
    ...
    func getCurrentSession(_ result: @escaping FlutterResult) -> Void {
        let currentSession = DeezerManager.sharedInstance.currentSession();
        let resultMap: NSDictionary = [
            "success": currentSession != nil,
            "accessToken": currentSession?.accessToken ?? NSNull(),
            "expires": currentSession?.expirationDate ?? NSNull(),
            "userId": currentSession?.userId ?? NSNull()
        ]
        result(resultMap)
    }
