...
typealias LoginResult = ((_ result: ResultLogin, _ data: ResultLoginData?) -> ())
...
class DeezerManager: NSObject {
    ...
    func login() {
        deezerConnect?.authorize([DeezerConnectPermissionBasicAccess, DeezerConnectPermissionEmail])
    }
    ...
    func currentSession() -> ResultLoginData? {
        let isValid = deezerConnect?.isSessionValid() ?? false
        if isValid {
            return ResultLoginData(userId: deezerConnect!.userId, accessToken: deezerConnect!.accessToken, expirationDate: Int((deezerConnect!.expirationDate.timeIntervalSince1970 * 1000.0).rounded()))
        }
        return nil
    }
}
...
extension DeezerManager: DeezerSessionDelegate {
    ...
    func deezerDidLogin() {
        ...
        loginResult?(.success, ResultLoginData(userId: deezerConnect.userId, accessToken: deezerConnect.accessToken, expirationDate: Int((deezerConnect.expirationDate.timeIntervalSince1970 * 1000.0).rounded())))
    }
    
    func deezerDidNotLogin(_ cancelled: Bool) {
        let deezerError: Error? = cancelled ? nil : NSError.instance(type: .noConnection)
        loginResult?(.error(error: deezerError), nil)
    }
    
    func deezerDidLogout(){
        clearTokenAndExpirationDate()
        loginResult?(.logout, nil)
    }
}
